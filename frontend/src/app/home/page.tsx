...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white text-slate-950">
      ...
      <section id="admin" className="m-3 grid grid-cols-[180px_1fr] overflow-hidden rounded-lg border border-slate-200 max-lg:grid-cols-1">
        <aside className="grid content-start gap-2 border-r border-slate-200 bg-slate-50 p-5"><strong>Admin Preview</strong>{["Dashboard", "Orders", "Products", "Customers", "Inventory", "Reports"].map((item) => <a className="rounded-lg px-3 py-2 font-semibold first:bg-slate-900 first:text-white" href="#admin" key={item}>{item}</a>)}</aside>
        <div className="p-5">
          <h2 className="text-2xl font-black">Dashboard Overview</h2>
          <div className="my-5 grid grid-cols-4 gap-4 max-lg:grid-cols-2 max-md:grid-cols-1">{[["Total Revenue", "฿125,430", "+18.4%"], ["Orders", "1,234", "+12.4%"], ["Customers", "849", "+9.7%"], ["Avg. Order Value", "฿1,016", "+5.3%"]].map(([label, value, delta]) => <article className="rounded-lg border border-slate-200 p-4" key={label}><p className="text-sm text-slate-500">{label}</p><strong className="mt-2 block text-2xl">{value}</strong><span className="text-sm font-bold text-emerald-700">{delta}</span></article>)}</div>
          <div className="grid grid-cols-2 gap-4 max-lg:grid-cols-1">
            <article className="rounded-lg border border-slate-200 p-5"><h3 className="font-black">Revenue Over Time</h3><div className="mt-4 h-36 rounded-lg bg-[linear-gradient(145deg,transparent_48%,#10964f_49%_51%,transparent_52%),repeating-linear-gradient(to_top,#fff_0_34px,#edf2f7_35px_36px)]" /></article>
            <article className="rounded-lg border border-slate-200 p-5"><h3 className="font-black">Recent Orders</h3><Table headers={["Order", "Customer", "Status"]} rows={orderRows.map((row) => [row[0], row[1], row[3]])} /></article>
            <article className="rounded-lg border border-slate-200 p-5"><h3 className="font-black">Product Management</h3><Table headers={["Product", "Category", "Stock", "Action"]} rows={products.slice(0, 4).map((product) => [product.name, product.category, String(product.stock), "Edit"])} /></article>
            <article className="rounded-lg border border-slate-200 p-5"><h3 className="font-black">Order Management</h3><Table headers={["Order", "Payment", "Fulfillment", "Action"]} rows={orderRows.map((row) => [row[0], row[2], row[3], "Update"])} /></article>
          </div>
        </div>
      </section>
